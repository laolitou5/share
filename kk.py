...
def additive_multiplicative_decomposition(df, code):
    ...
    try:
        ...
        plt.savefig('trend_plot/%s.png'%code)
        #plt.close()
        # plt.show()
        itype  = 'add'
        if itype == 'mul':
...
if __name__ == '__main__':
    ...
    df = pd.read_csv('000601.SZ.csv', parse_dates=['date'], index_col='date')
    
    #df.index = df.index.astype(object)
    #df.sort_index(inplace=True)

    additive_multiplicative_decomposition(df, '000601.SZ')
